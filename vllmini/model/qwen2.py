...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        q = q.reshape(batch_size, seq_len, self.num_attention_heads, self.head_dim).transpose(1, 2)
        k = k.reshape(batch_size, seq_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        v = v.reshape(batch_size, seq_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
    ...
    def _paged_attention(self, q: torch.Tensor, key_cache: torch.Tensor, value_cache: torch.Tensor, 
        ...
        return out_flat.reshape(num_seqs, num_heads, seq_len, head_dim)
